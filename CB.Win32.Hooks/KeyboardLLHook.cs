...
{
    public class KeyboardLLHook: HookBase
    {
        #region  Constructors & Destructor
        public KeyboardLLHook()
            : base(HookTypes.KeyboardLowLevel)
        {
            Procedure = new HookProc(KeyboardLLProc);
        }
        #endregion
        ...
        ///     calls this function every time a new keyboard input event is about to be posted into a thread input queue.
        ...
        ///     <para>HC_ACTION = 0. The wParam and lParam parameters contain information about a keyboard message.</para>
        /// </param>
        /// <param name="wParam">
        ...
        /// </remarks>
        protected virtual int KeyboardLLProc(int nCode, int wParam, int lParam)
        {
            if (nCode == HC_ACTION && Event != null)
            {
                var ki = new KeyboardLLHookInfo(nCode, wParam, lParam);
                Event(this, ki);
                if (ki.Handled)
                    return 1;
            }
